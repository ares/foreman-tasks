...
%build
sed -ri '1sX(/usr/bin/ruby|/usr/bin/env ruby)X%{scl_ruby}X' %{_builddir}/%{pkg_name}-%{version}/%{gem_instdir}/bin/foreman-jobs
...
ln -sv %{gem_instdir}/bin/%{jobs_name} %{buildroot}%{_bindir}/foreman-jobs
%else
...
ln -sv %{gem_instdir}/bin/%{jobs_name} %{buildroot}%{_root_bindir}/foreman-jobs
%if 0%{?rhel} == 6
...
%{_bindir}/foreman-jobs
%else
%{_root_bindir}/foreman-jobs
%if 0%{?rhel} == 6
